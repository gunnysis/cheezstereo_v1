...
export default function VideosScreen() {
  ...
  const fetchVideos = async (query: string = '', order: string = 'date') => {
    try {
      setError(null);
      const data = query
        ? await searchChannelVideos(YOUTUBE_CHANNELS.videos, query, 50, order)
        : await getChannelVideos(YOUTUBE_CHANNELS.videos, 50, order);
      setVideos(data);
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : '비디오를 불러오는데 실패했습니다.';
      setError(errorMessage);
      console.error('영상 목록 로드 에러:', err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  if (videos.length === 0) {
    ...
    return (
      ...
      >
        ...
        <EmptyState
          icon={isSearching ? "search-outline" : "videocam-outline"}
          title={isSearching ? "검색 결과 없음" : "영상이 없습니다"}
          description={
            isSearching
              ? `"${searchQuery}"에 대한 검색 결과를 찾을 수 없습니다.`
              : "치즈스테레오의 영상을 불러올 수 없습니다."
          }
